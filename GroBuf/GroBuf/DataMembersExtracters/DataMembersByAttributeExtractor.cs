    ...
    public class DataMembersByAttributeExtractor : IDataMembersExtractor
    {
        ...
        public Tuple<string, MemberInfo>[] GetMembers(Type type)
        {
            return (from property in type.GetProperties(BindingFlags.Public | BindingFlags.Instance)
                    ...
                    select new Tuple<string, MemberInfo>(dataMemberAttribute == null || string.IsNullOrEmpty(dataMemberAttribute.Name) ? property.Name : dataMemberAttribute.Name, property)).ToArray();
        }
    }
}